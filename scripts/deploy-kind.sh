...
deploy_stack() {
  ...
  echo ">>> Loading local load-generator image into Kind (if present)"
  # This assumes you have built the image locally with:
  #   docker build -f load-generator-image/Dockerfile.workload \
  #     -t adamkkk89/geth-workload:latest \
  #     load-generator-image
  kind load docker-image adamkkk89/geth-workload:latest --name "${KIND_CLUSTER_NAME}" || true
